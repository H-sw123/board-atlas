...
GitHub 仓库为 `waveshareteam/board-atlas`，在线组件库名称为 **`waveshare/board_atlas_boards`**，
...
`compote component pack --name board_atlas_boards` 可以在无令牌情况下验证本地打包，
...
3. 从该标签运行 **Publish board pack**，取消勾选 **dry_run**。
   ...
   再上传 `waveshare/board_atlas_boards`。
